...
function sendItToServer(orcid) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        if (
          xhr.responseText === "BadArgument" ||
          xhr.responseText === "SomethingFailed"
        ) {
          ...
          if (response.account == "") {
            ...
            document.getElementById("result").innerHTML =
              "<p>Please login into your Ethereum Address in MetaMask</p>";
          } else if (response.orcid == "") {
            //show the result in html page
            document.getElementById("result").innerHTML =
              "<p>Please re-connect to your ORCID iD</p>";
          } else {
            //show the result in html page
            document.getElementById("result").innerHTML =
              "<p>Logged in Ethereum Address = " +
              response.account +
              "</p>" +
              "<p>orcid = " +
              response.orcid +
