    ...
    Messages: [
      {
        ...
        TextPart: `Hello,\n\nPlease verify your account by clicking the link:\n${
          process.env.EXPRESS_ADDRESS
        }/confirm/${iv.toString("hex")}/${encrypt(
          token,
          iv
...
const getName = async (groupInstance, userAddress, next) => {
  let name;
  await groupInstance.methods
    .getName()
    .call({ from: userAddress }, (err, result) => {
      if (err) {
        return next(err);
      }
...
const checkOwnerRemovalVote = async (
  contractInstance,
  address,
  userAddress,
  next
...
const getGroupInfo = async (
  ...
  if (resData.isOwner) {
    ...
    for (let i = 0; i < ownerCount; i++) {
      ...
      resData.owners.push({
        address,
        isVoted: await checkOwnerRemovalVote(
          contractInstance,
          address,
          userAddress,
          next
...
router.get("/", session.isSignedIn, async (req, res, next) => {
  const resData = await getGroupInfo(
    "Root",
...
router.get("/group/:address", session.isSignedIn, async (req, res, next) => {
  ...
  const resData = await getGroupInfo(
    await getName(groupInstance, req.user.address, next),
    req.params.address,
    groupInstance,
    req.session.breadcrumb,
    req.user.address,
...
router.get("/signin", session.isSignedOut, async (req, res) => {
  ...
  let orcid = "";
  if (req.query.code) {
    await authenticateOrcid(req.query.code, "signin")
      .then((result) => {
        orcid = result;
      })
      .catch((err) => {
  ...
  res.render("register", {
    ...
    redirectUri: process.env.EXPRESS_ADDRESS + "/signin",
  });
});

router.post("/signin", session.isSignedOut, (req, res, next) => {
  session.passport.authenticate(
    req.body.email ? "local-email" : "local-orcid",
    (err, user, info) => {
      if (err) {
        return next(err);
      }
...
router.get("/signup", session.isSignedOut, async (req, res) => {
  ...
  let orcid = "";
  if (req.query.code) {
    await authenticateOrcid(req.query.code, "signup")
      .then((result) => {
        orcid = result;
      })
      .catch((err) => {
  ...
  res.render("register", {
    ...
    orcid,
    clientId: process.env.ORCID_CLIENT_ID,
    redirectUri: process.env.EXPRESS_ADDRESS + "/signup",
  });
});
...
router.post("/signup", session.isSignedOut, (req, res, next) => {
  ...
  models.User.findOne(
    ...
    (err, user) => {
      ...
      if (user) {
        req.session.message =
          "This Ethereum address is already associated with another account.";
      ...
      if (req.body.email) {
        ...
        user.save((err) => {
          ...
          web3.rootInstance.methods
            .addMember(web3.web3.utils.fromAscii("Member"), user.address)
            .send(
              { from: process.env.ROOT_CONTRACT_OWNER_ADDRESS },
              (err, transactionHash) => {
                if (err) {
                  return next(err);
                }
                req.session.message =
                  "Your account has successfully created. You can now sign in.";
                res.redirect("/signin");
              }
            );
        });
